...
// Log paths and status only: credentials may appear in bodies or query strings.
const logStream = fs.createWriteStream(path.join(__dirname, 'access.log'), { flags: 'a' });
morgan.token('safe-path', (req: Request) => req.path);
app.use(morgan(':method :safe-path :status :response-time ms', { stream: logStream }));
