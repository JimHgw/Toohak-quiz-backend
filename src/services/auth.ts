...
// Local demo fallback; never commit an actual signing secret.
const jwtSecretKey = process.env.JWT_SECRET || randomBytes(32).toString('hex');
...
export function adminAuthRegister(email: string, password: string, nameFirst: string, nameLast: string): { token: string } {
  ...
  const token = jwt.sign({ payload: Math.random() }, jwtSecretKey);
